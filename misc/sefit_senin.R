    ...
    for(v in 1:p)
      {
        i <- ij[u,1]; j <- ij[u,2]
        l <- ij[v,1]; m <- ij[v,2]
        new1 <-  n * ( QQ[j,m]%*%d1[l,i] + AA[m,i]%*%AA[j,l] 
        + 2%*%e21_1[m,i]%*% e21_1[j,l] - 2%*%e21_2[m,i]%*% e21_2[j,l] )          # koreksi 1
        k = c(k,new1)
        }
    }               
#----------------------------------------------------------------------------
...
seA <- sqrt(diag(kinv))
# for delta
uu= n * (0.5*(Delta %*%Delta) - 0.5* (e24_1%*% e24_1) + (1/2) *( e24_2%*% e24_2 ))    # koreksi 2
uut=1/diag(uu)
seD=sqrt(uut)
DD=Dhat/seD
DD=as.vector(DD)
...
names(TT) <- names(seA)                                       
PP<-2*(1-pnorm(abs(TT)))
tab <-round(cbind(Estimated = 
pp, s.e. = seA, z_val = TT, p_val = PP),6)                               
